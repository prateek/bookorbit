...
export class ScanGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect, OnModuleInit, OnModuleDestroy {
  ...
  async handleSubscribeLibrary(client: Socket, rawLibraryId: unknown): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(
        `[scanner.ws_subscribe] [fail] userId=${user?.id ?? 0} libraryId=${libraryId} socketId=${client.id} errorClass=${err instanceof Error ? err.name : 'Error'} error="${sanitizeLogValue(err instanceof Error ? err.message : String(err))}" - library access check failed`,
      );
      return;
    }
    if (!allowed) {
      this.logger.warn(
        `[scanner.ws_subscribe] [fail] userId=${user?.id ?? 0} libraryId=${libraryId} socketId=${client.id} errorClass=ForbiddenException error="library access denied" - library subscription rejected`,
      );
      return;
    }
