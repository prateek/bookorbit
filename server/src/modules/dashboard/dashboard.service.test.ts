...
function standaloneGroups(bookIds: number[]) {
  return bookIds.map((bookId) => ({ bookId, seriesId: null, bookIds: [bookId], latestAddedAt: new Date('2026-01-01T00:00:00.000Z') }));
}
...
describe('DashboardService', () => {
  ...
  it('folds several new entries of one series into a single recently added card', async () => {
    const { service, dashboardRepo, bookReadService, libraryService } = makeService();
    const user = makeUser({ id: 8 });
    libraryService.findAccessibleLibraryIds.mockResolvedValue([10]);
    dashboardRepo.findRecentlyAddedGroups.mockResolvedValue([
      { bookId: 21, seriesId: 4, bookIds: [21, 22, 23], latestAddedAt: new Date('2026-02-01T00:00:00.000Z') },
      { bookId: 9, seriesId: null, bookIds: [9], latestAddedAt: new Date('2026-01-01T00:00:00.000Z') },
    ]);
    ...
    expect(recent?.books[0]?.collapsedSeries).toMatchObject({ bookCount: 3, firstVolumeBookId: 21, latestVolumeBookId: 23 });
