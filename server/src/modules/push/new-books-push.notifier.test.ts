...
describe('NewBooksPushNotifier', () => {
  ...
  it('requeues a batch that failed before delivery and retries it with newly queued books', async () => {
    repo.summarizeNewBooks.mockRejectedValueOnce(new Error('db down'));
    notifier.enqueue(1, [11]);
    await notifier.flush();
    ...
    expect(repo.summarizeNewBooks).toHaveBeenCalledTimes(2);
    expect(repo.summarizeNewBooks).toHaveBeenLastCalledWith([11, 20]);
    expect(pushService.deliver).toHaveBeenCalledTimes(1);
  });

  it('gives up on a batch after repeated failures', async () => {
    repo.summarizeNewBooks.mockRejectedValue(new Error('db down'));
    notifier.enqueue(1, [11]);
    await notifier.flush();
    await vi.advanceTimersByTimeAsync(30_000);
    await vi.advanceTimersByTimeAsync(30_000);
    expect(repo.summarizeNewBooks).toHaveBeenCalledTimes(3);

    await vi.advanceTimersByTimeAsync(5 * 60_000);
    expect(repo.summarizeNewBooks).toHaveBeenCalledTimes(3);
  });

  it('does not requeue once delivery has started', async () => {
    repo.summarizeNewBooks.mockResolvedValue([{ libraryId: 1, seriesId: 7, seriesName: 'Chrysalis', count: 1, sampleBookId: 11 }]);
    repo.findRecipientsForLibrary.mockResolvedValue([recipient(100, 1)]);
    pushService.deliver.mockRejectedValueOnce(new Error('push service down'));
    notifier.enqueue(1, [11]);
    await notifier.flush();
  ...
  it('flushes queued books on shutdown instead of dropping them', async () => {
    notifier.enqueue(1, [11]);
    await notifier.onModuleDestroy();
    expect(repo.summarizeNewBooks).toHaveBeenCalledWith([11]);
    expect(pushService.deliver).toHaveBeenCalledTimes(1);
  });

  it('stops waiting for a hung flush when the shutdown budget runs out', async () => {
    repo.summarizeNewBooks.mockReturnValue(new Promise(() => {}));
    notifier.enqueue(1, [11]);
    const warn = vi.spyOn(Logger.prototype, 'warn').mockImplementation(() => undefined);
    const destroyed = notifier.onModuleDestroy();
    await vi.advanceTimersByTimeAsync(10_000);
    await expect(destroyed).resolves.toBeUndefined();
    expect(warn).toHaveBeenCalledWith(expect.stringContaining('[push.new_books_shutdown] [fail]'));
  });

  it('sends a full batch right away so books from other libraries are still queued', async () => {
    const firstLibrary = Array.from({ length: 50_000 }, (_, i) => i + 1);
    notifier.enqueue(1, firstLibrary);
    notifier.enqueue(1, [50_001]);
    notifier.enqueue(2, [90_000]);
    expect(repo.summarizeNewBooks).toHaveBeenCalledTimes(1);
    expect(repo.summarizeNewBooks.mock.calls[0][0]).toHaveLength(50_000);

    await vi.advanceTimersByTimeAsync(30_000);
    expect(repo.summarizeNewBooks).toHaveBeenCalledTimes(2);
    expect(repo.summarizeNewBooks).toHaveBeenLastCalledWith([50_001, 90_000]);
  });
});
