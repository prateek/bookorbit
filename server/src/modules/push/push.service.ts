...
export class PushService {
  ...
  async subscribe(userId: number, dto: SavePushSubscriptionDto, userAgent: string | null): Promise<void> {
    ...
    this.logger.log(`[push.subscribe] [end] userId=${userId} - push subscription saved`);
  }
  ...
  async unsubscribe(userId: number, endpoint: string): Promise<void> {
    ...
    this.logger.log(`[push.unsubscribe] [end] userId=${userId} deleted=${deleted} - push subscription removed`);
  }
  ...
  async deliver(recipients: Array<{ recipient: PushRecipient; payload: PushPayload }>): Promise<PushDeliveryResult> {
    ...
    await mapWithConcurrency(recipients, SEND_CONCURRENCY, async ({ recipient, payload }) => {
      try {
        ...
        const statusCode = (err as { statusCode?: unknown } | null)?.statusCode;
        this.logger.warn(
          `[push.send] [fail] userId=${recipient.userId} subscriptionId=${recipient.subscriptionId} statusCode=${typeof statusCode === 'number' ? statusCode : 'none'} errorClass=${err instanceof Error ? err.name : 'Error'} error="${sanitizeLogValue(err instanceof Error ? err.message : String(err))}" - push delivery failed`,
        );
      }
    });
