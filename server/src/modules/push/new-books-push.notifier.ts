...
/** A batch this large is sent right away so memory stays bounded during a first import. */
const MAX_PENDING_BOOK_IDS = 50_000;
/** A batch that fails before delivery is retried this many times before it is dropped. */
const MAX_FLUSH_ATTEMPTS = 3;
/** Well inside docker's 30s stop_grace_period. */
...
export class NewBooksPushNotifier implements OnModuleDestroy {
  ...
  async onModuleDestroy(): Promise<void> {
    ...
    const timedOut = new Promise<boolean>((resolve) => {
      timeout = setTimeout(() => resolve(true), SHUTDOWN_FLUSH_TIMEOUT_MS);
    });
    const drained = Promise.allSettled([...this.inFlight, this.flush()]).then(() => false);
    try {
      if (await Promise.race([drained, timedOut])) {
        this.logger.warn(
          `[push.new_books_shutdown] [fail] durationMs=${Date.now() - startedAt} errorClass=TimeoutError error="flush did not finish before shutdown" - pending new book pushes may be lost`,
        );
      }
    } finally {
  ...
  enqueue(libraryId: number, bookIds: readonly number[]): void {
    ...
    for (const id of bookIds) {
      if (this.pendingCount >= MAX_PENDING_BOOK_IDS) void this.flush();
      this.add(libraryId, id);
    }
    this.schedule();
  }
  ...
  async flush(): Promise<void> {
    ...
    const attempt = this.pendingAttempts + 1;
    ...
    const run = this.send(batch, attempt);
  ...
  private async send(batch: Map<number, Set<number>>, attempt: number): Promise<void> {
    const event = 'push.new_books';
    const startedAt = Date.now();
    const bookIds = [...batch.values()].flatMap((ids) => [...ids]);
    this.logger.log(`[${event}] [start] libraryCount=${batch.size} bookCount=${bookIds.length} attempt=${attempt} - new book push started`);
    ...
    try {
      ...
      const requeued = !delivering && attempt < MAX_FLUSH_ATTEMPTS && !this.stopping;
      const droppedCount = requeued ? this.requeue(batch, attempt) : bookIds.length;
      this.logger.warn(
        `[${event}] [fail] libraryCount=${batch.size} bookCount=${bookIds.length} durationMs=${Date.now() - startedAt} attempt=${attempt} requeued=${requeued} droppedCount=${droppedCount} errorClass=${err instanceof Error ? err.name : 'Error'} error="${sanitizeLogValue(err instanceof Error ? err.message : String(err))}" - new book push failed`,
      );
    }
  }

  private add(libraryId: number, bookId: number): boolean {
    let ids = this.pending.get(libraryId);
    if (ids?.has(bookId)) return true;
    if (this.pendingCount >= MAX_PENDING_BOOK_IDS) return false;
    if (!ids) {
      ids = new Set();
  ...
  /** Returns how many books did not fit back into the queue. */
  private requeue(batch: Map<number, Set<number>>, attempt: number): number {
    let dropped = 0;
    for (const [libraryId, ids] of batch) for (const id of ids) if (!this.add(libraryId, id)) dropped++;
    this.pendingAttempts = Math.max(this.pendingAttempts, attempt);
    this.schedule();
    return dropped;
  }

  private schedule(): void {
    if (this.stopping || this.pendingCount === 0) return;
    const now = Date.now();
