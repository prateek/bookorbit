...
const mocks = vi.hoisted(() => ({
  ...
  route: null as unknown as { name: string; fullPath: string },
}))

vi.mock('vue-router', async (importOriginal) => {
  const actual = await importOriginal<typeof import('vue-router')>()
  const { reactive } = await import('vue')
  mocks.route = reactive({ name: 'dashboard', fullPath: '/' })
  return {
    ...actual,
...
describe('AppHeader global search', () => {
  ...
  it('drops a pending Enter when the route changes before results arrive', async () => {
    mocks.results = ref([])
    const wrapper = mountHeader()
    const input = wrapper.get('input[placeholder="Search all books..."]')
