...
describe('useSeriesDetail', () => {
  ...
  describe('jumping into a long series', () => {
    ...
    it('refreshes a jumped run in place, picking up new chapters on its last page', async () => {
      mockFetchSeriesBooks
        .mockResolvedValueOnce(page([501], 10))
        .mockResolvedValueOnce(page([551], 11))
        .mockResolvedValueOnce({ ...page([501], 10), total: 1702 })
        .mockResolvedValueOnce({ ...page([551, 1701, 1702], 11), total: 1702 })

      const { items, total, loading, firstIndex, load, jumpTo, refresh } = useSeriesDetail(ref(42))
      await jumpTo(501)
      await load()
      ...
      expect(mockFetchSeriesBooks).toHaveBeenNthCalledWith(4, 42, expect.objectContaining({ page: 11 }))
      expect(items.value.map((b) => b.id)).toEqual([501, 551, 1701, 1702])
