...
export function useSeriesDetail(seriesId: Ref<number | null>) {
  ...
  /**
   * Refetches the first page of the loaded run in place and drops the pages after it, so a resume
   * costs one request however far the reader had scrolled; infinite scroll reloads the rest. A
   * jumped run keeps its starting page. Any load that starts meanwhile wins.
   */
  async function refresh(): Promise<void> {
    ...
    try {
      const data = await fetchSeriesBooks(currentSeriesId, queryFor(startPage))
